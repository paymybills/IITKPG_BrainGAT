...
\begin{itemize}[noitemsep]
  ...
  \item \texttt{scripts/}: utility scripts for data download, model training, and robustness validation.
  ...
  \item \texttt{new/}: main working directory containing the final \texttt{BrainGAT\_Evolution.ipynb} notebook, trained models, and this report.
\end{itemize}

Within this structure, most of the internship activity centred on the architecture documents in \texttt{docs/} and the BrainGAT Evolution training and analysis workflow.
...
The first major architecture studied was BrainGNN~\cite{li2021braingnn}, documented in \texttt{docs/BrainGNN\_Architecture.md}. BrainGNN is an interpretable brain graph neural network with several key ideas:
\begin{itemize}[noitemsep]
...
\paragraph{Original BrainGAT (Static GAT).} In addition to the documentation in \texttt{docs/BrainGAT\_Architecture.md}, I worked with the fixed-size training pipeline and related model checkpoints. These experiments reinforced:
\begin{itemize}[noitemsep]
