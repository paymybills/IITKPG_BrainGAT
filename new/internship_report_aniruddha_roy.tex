...
%-------------------- Abstract --------------------
\begin{abstract}
Autism Spectrum Disorder (ASD) is a complex neurodevelopmental condition whose diagnosis is traditionally based on behavioural assessments. This internship explored the feasibility of computer-assisted ASD screening using resting-state functional MRI (rs-fMRI) from the ABIDE (Autism Brain Imaging Data Exchange) dataset, formulated as a subject-level brain graph classification problem. Within an existing research workspace for brain graph neural networks (GNNs), I investigated several architectures, implemented a new heterogeneous temporal--spatial Graph Attention Network (BrainGAT Evolution) for purely imaging-based ASD classification (without phenotype covariates), and carefully refined the evaluation protocol.

The work progressed through: (i) understanding and reproducing baseline models such as BrainGNN and the original BrainGAT; (ii) extending the architecture to jointly model temporal dynamics and multi-scale spatial connectivity with 4-dimensional edge features; (iii) refining the evaluation protocol, including correcting an early window-level splitting issue that had produced overly optimistic validation accuracy; and (iv) conducting structured hyperparameter search and triple-set evaluation (validation, Test1, Test2) to obtain honest, generalizable performance estimates.

The final BrainGAT Evolution model achieves realistic validation and test accuracies in the range expected for state-of-the-art ABIDE models (approximately 60--70\%), with strict subject-level splitting and comprehensive checks for overlap across train, validation, and test subjects. This report summarises the problem statement, the solution architectures studied and developed, investigations carried out, issues encountered and their fixes, and the resulting final model.
\end{abstract}
...
%-------------------- Repository --------------------
\noindent\textbf{GitHub Repository:} \url{https://github.com/paymybills/IITKPG_BrainGAT}
...
\begin{itemize}[noitemsep]
  ...
  \item to design and implement a new heterogeneous temporal--spatial Graph Attention Network (BrainGAT Evolution) that operates purely on imaging-derived brain graphs (without phenotype inputs) while integrating temporal dynamics and multi-scale spatial connectivity;
  \item to design, debug, and stabilise the evaluation protocol (including subject-level splitting) so that reported results are realistic; and
...
\begin{itemize}[noitemsep]
  \item \href{https://github.com/paymybills/IITKPG_BrainGAT/tree/master/abide_data}{\texttt{abide\_data/}}: ABIDE time series and derivatives (e.g., ROI signals in CC400 atlas, CPAC outputs).
  \item \href{https://github.com/paymybills/IITKPG_BrainGAT/tree/master/notebooks}{\texttt{notebooks/}}: exploratory and experimental Jupyter notebooks (e.g., early BrainGNN and BrainGAT experiments).
  \item \href{https://github.com/paymybills/IITKPG_BrainGAT/tree/master/models}{\texttt{models/}}: reusable model definitions, including a DMSGCN implementation.
  \item \href{https://github.com/paymybills/IITKPG_BrainGAT/tree/master/scripts}{\texttt{scripts/}}: utility scripts for data download, model training, and robustness validation.
  \item \href{https://github.com/paymybills/IITKPG_BrainGAT/tree/master/docs}{\texttt{docs/}}: architecture documentation for BrainGNN, BrainGAT, and BrainGAT Evolution, as well as a dedicated data-leakage report.
  \item \href{https://github.com/paymybills/IITKPG_BrainGAT/tree/master/env}{\texttt{env/}}: environment files (\texttt{requirements.txt}, CUDA-specific setup script) for reproducible experimentation.
  \item \href{https://github.com/paymybills/IITKPG_BrainGAT/tree/master/new}{\texttt{new/}}: main working directory containing the final \texttt{BrainGAT\_Evolution.ipynb} notebook, trained models, and this report.
\end{itemize}

Within this structure, most of the internship activity centred on the architecture documents in \texttt{docs/} and the \href{https://github.com/paymybills/IITKPG_BrainGAT/blob/master/new/BrainGAT_Evolution.ipynb}{BrainGAT Evolution training and analysis workflow}.
...
The Autism Brain Imaging Data Exchange (ABIDE)~\cite{di2014autism} aggregates rs-fMRI data from more than 20 international sites, with over 1{,}000 subjects. In this workspace, the following choices are used consistently:
\begin{itemize}[noitemsep]
...
The first major architecture studied was BrainGNN~\cite{li2021braingnn}, documented in \href{https://github.com/paymybills/IITKPG_BrainGAT/blob/master/docs/BrainGNN_Architecture.md}{\texttt{docs/BrainGNN\_Architecture.md}}. BrainGNN is an interpretable brain graph neural network with several key ideas:
\begin{itemize}[noitemsep]
...
The second baseline, documented in \texttt{docs/BrainGAT\_Architecture.md}, is BrainGAT: a pure imaging-based Graph Attention Network~\cite{velivckovic2017graph} for ASD classification. Its main characteristics are:
\begin{itemize}[noitemsep]
...
\paragraph{BrainGNN (ROI-aware GNN).} Using \href{https://github.com/paymybills/IITKPG_BrainGAT/blob/master/docs/BrainGNN_Architecture.md}{\texttt{docs/BrainGNN\_Architecture.md}}, \href{https://github.com/paymybills/IITKPG_BrainGAT/blob/master/docs/braingnn_architecture.tex}{\texttt{braingnn\_architecture.tex}}, and the associated notebooks (notably \href{https://github.com/paymybills/IITKPG_BrainGAT/blob/master/notebooks/BrainGNN_repro.ipynb}{\texttt{notebooks/BrainGNN\_repro.ipynb}}), I studied and partially reproduced BrainGNN, focusing on:
\begin{itemize}[noitemsep]
...
\paragraph{Original BrainGAT (Static GAT).} In addition to the documentation in \href{https://github.com/paymybills/IITKPG_BrainGAT/blob/master/docs/BrainGAT_Architecture.md}{\texttt{docs/BrainGAT\_Architecture.md}}, I worked with the fixed-size training pipeline and related model checkpoints. These experiments reinforced:
\begin{itemize}[noitemsep]
...
\paragraph{DMSGCN and Population-level Models.} The DMSGCN architecture~\cite{wen2022dmsgcn} implements a population-graph-based GCN inspired by multi-site learning and leave-one-site-out (LOSO) or 10-fold cross-validation protocols. I examined this approach to understand:
\begin{itemize}[noitemsep]
...
\paragraph{Exploratory Notebooks.} Several experimental notebooks under \texttt{notebooks/} were used to prototype, sanity-check, and compare different GNN variants. In these, I experimented with:
\begin{itemize}[noitemsep]
...
\paragraph{Kaggle-optimised BrainGAT.} A simplified and resource-aware version of BrainGAT was developed for Kaggle GPUs. I reviewed this implementation to understand practical engineering strategies (reduced feature dimensions, careful batch sizing, and checkpointing) that later informed the design of BrainGAT Evolution training loops.
...
BrainGAT Evolution integrates a temporal CNN-attention branch with a multi-scale spatial GAT branch that uses 4D edge features, followed by late fusion. The resulting architecture is heterogeneous across temporal and spatial components and operates exclusively on imaging-derived graphs, without including phenotype or site-level covariates.
...
%-------------------- 5. Evaluation Protocol and Data Splitting --------------------
\section{Evaluation Protocol and Data Splitting}

\subsection{Initial Splitting Issue}

An early version of the BrainGAT Evolution pipeline exhibited unusually high validation accuracy (around 95\%), which is higher than typical published ABIDE results (71--74\% for BrainGNN and typically 60--70\% for strong baselines). This prompted a closer inspection of the evaluation protocol.

The main issue was window-level splitting:
\begin{itemize}[noitemsep]
  ...
  \item As a result, windows from the same subject could appear in both training and validation sets, effectively leaking subject-specific information between splits.
\end{itemize}

This configuration leads to overestimation of performance because the model effectively sees the same subject in both training and validation, a common pitfall when working with temporal windows in neuroimaging.

\subsection{Subject-Level Splitting}

To address this, the pipeline was updated to use strict subject-level splitting, as implemented in \texttt{BrainGAT\_Evolution.ipynb} and described in the accompanying documentation:
\begin{enumerate}[noitemsep]
...
Importantly, BrainGAT Evolution is a purely imaging-based, heterogeneous graph attention network: all inputs are derived from rs-fMRI-based brain graphs, with no explicit phenotype or site features.

\subsection{Performance Summary}

After refining the evaluation protocol and stabilising training, the model achieves:
\begin{itemize}[noitemsep]
...
Quantitatively, these results are consistent with the expectations documented in \href{https://github.com/paymybills/IITKPG_BrainGAT/blob/master/docs/DATA_LEAKAGE_FIX.md}{\texttt{docs/DATA\_LEAKAGE\_FIX.md}} and \href{https://github.com/paymybills/IITKPG_BrainGAT/blob/master/docs/BrainGAT_Evolution_Architecture.md}{\texttt{docs/BrainGAT\_Evolution\_Architecture.md}}, where realistic ABIDE performance is expected to lie between 60--72\% for strong models, and where previously observed 95\% accuracy was explicitly labelled as an artefact of leakage.
...
\subsubsection{Implemented Visualizations}

During the internship, the following visualizations were successfully generated:
\begin{itemize}[noitemsep]
  \item Training and validation curves (Figure~\ref{fig:training_curves})
  \item \textbf{Biomedical interpretability visualizations} (Figures~\ref{fig:brain_connections}, \ref{fig:roi_importance}, \ref{fig:hemisphere}, \ref{fig:connectivity})
  \item Hyperparameter sensitivity analysis (stored in \texttt{sweep\_results.json})
\end{itemize}

\subsubsection{Biomedical Interpretability Analysis}

Following model training, a comprehensive biomedical interpretability analysis was conducted to understand \emph{what} the model learned from a neuroscience perspective. This analysis is critical for clinical translation and validation against established ASD neuroscience.

\paragraph{Top Discriminative Brain Connections.}
Figure~\ref{fig:brain_connections} shows the top 25 brain connections (ROI-ROI pairs) that exhibit the largest connectivity differences between ASD and control groups. These connections were identified by computing the absolute difference between group-averaged correlation matrices.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{top_brain_connections.png}
  \caption{Top 25 most discriminative brain connections for ASD classification. Connections are ranked by absolute correlation difference between ASD and control groups. Higher values indicate stronger group differences.}
  \label{fig:brain_connections}
\end{figure}

\paragraph{ROI Importance Mapping.}
ROI importance was computed by summing the discriminative power of all connections involving each brain region. Figure~\ref{fig:roi_importance} displays the top 30 brain regions, with ROI 204 showing the highest importance (normalized score = 1.000), followed by ROI 069 (0.974) and ROI 247 (0.970).

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{roi_importance_map.png}
  \caption{Top 30 brain regions (ROIs) ranked by importance for ASD classification. Importance scores represent the cumulative discriminative power of connections involving each region. These ROIs represent potential neuroimaging biomarkers for ASD.}
  \label{fig:roi_importance}
\end{figure}

\paragraph{Hemispheric Lateralization Analysis.}
Figure~\ref{fig:hemisphere} presents the hemispheric contribution analysis. The results show relatively balanced contributions: left hemisphere 52.7\% (16 important ROIs), right hemisphere 47.3\% (14 important ROIs), with only 5.3\% asymmetry. This balanced bilateral pattern suggests that ASD-related connectivity alterations are distributed across both hemispheres rather than strongly lateralized.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{hemisphere_analysis.png}
  \caption{Hemispheric contribution to ASD classification. (Left) Pie chart showing percentage contribution of each hemisphere. (Right) Average ROI importance per hemisphere. The balanced distribution (5.3\% asymmetry) indicates distributed bilateral pathology consistent with ASD as a network-level disorder.}
  \label{fig:hemisphere}
\end{figure}

\paragraph{Functional Connectivity Comparison.}
Figure~\ref{fig:connectivity} displays the group-averaged functional connectivity matrices for ASD and control subjects, along with their difference map. Key findings include:
\begin{itemize}[noitemsep]
  \item Mean correlation: ASD = 0.222, Control = 0.221 (similar overall connectivity strength)
  \item 3,780 connections show $|\Delta| > 0.3$ (substantial group differences)
  \item Maximum absolute difference = 0.653 (specific circuit alterations)
  \item Difference map reveals scattered alterations across the connectivity matrix, consistent with distributed network dysfunction
\end{itemize}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{connectivity_matrices.png}
  \caption{Functional connectivity matrices: (a) Average connectivity for ASD group, (b) Average connectivity for control group, (c) Difference map (ASD $-$ Control) highlighting altered connectivity patterns. Red indicates stronger connectivity in ASD, blue indicates stronger connectivity in controls. The scattered pattern of differences supports the hypothesis of distributed network alterations in ASD.}
  \label{fig:connectivity}
\end{figure}

\paragraph{Clinical Interpretation.}
The biomedical interpretability analysis reveals several clinically relevant findings:
\begin{itemize}[noitemsep]
  \item \textbf{Distributed pathology}: The 5.3\% hemispheric asymmetry and scattered connectivity differences support ASD as a network-level disorder affecting distributed circuits rather than focal regions.
  \item \textbf{Potential biomarkers}: The top 30 identified ROIs represent candidate neuroimaging biomarkers that should be validated in independent cohorts and mapped to canonical brain networks (default mode, salience, executive control).
  \item \textbf{Connectivity alterations}: The 3,780 significantly different connections may reflect the known ASD patterns of reduced long-range connectivity and altered local circuit organization.
  \item \textbf{Future validation}: These findings should be compared against established ASD neuroscience literature, particularly regarding DMN alterations, social brain network dysfunction, and atypical brain maturation.
\end{itemize}
...
A systematic grid search was conducted over 16 hyperparameter configurations, varying learning rate (0.0001--0.001), weight decay (0.0001--0.001), label smoothing (0.0--0.1), and dropout (0.2--0.4). The results are stored in \texttt{sweep\_results.json}. The optimal configuration was found to be: learning rate = 0.001, weight decay = 0.0005, label smoothing = 0.05, dropout = 0.3, achieving 63.75\% validation accuracy.

\subsection{Qualitative Observations}

From the training and validation curves generated in the notebook, several patterns emerge:
\begin{itemize}[noitemsep]
  ...
  \item Multi-scale connectivity and temporal modelling reduce overfitting compared to purely static models, as evidenced by the ablation studies.
\end{itemize}

\subsection{Biomedical Validation and Clinical Significance}

The biomedical interpretability analysis (Figures~\ref{fig:brain_connections}--\ref{fig:connectivity}) provides crucial neuroscience validation of the model's learned representations. The key findings---30 discriminative brain regions, balanced hemispheric contribution, and 3,780 altered connections---align with current understanding of ASD as a distributed network disorder.

\paragraph{Strengths of the Current Analysis:}
\begin{itemize}[noitemsep]
  \item Identified specific candidate neuroimaging biomarkers (top ROIs)
  \item Demonstrated balanced bilateral pathology consistent with ASD neuroscience
  \item Quantified connectivity alterations between ASD and control groups
  \item Provided interpretable visualizations for clinical validation
\end{itemize}

\paragraph{Remaining Interpretability Challenges:}
While the connectivity-based analysis provides valuable insights, several advanced interpretability methods remain for future work:
\begin{itemize}[noitemsep]
  \item \textbf{Direct GAT Attention Extraction}: Implementing layer hooks to extract actual attention weights from the trained GAT layers, rather than using correlation differences as a proxy.
  \item \textbf{Functional Network Mapping}: Mapping identified ROIs to canonical brain networks (default mode, salience, executive control) using established brain atlases and network parcellations.
  \item \textbf{Anatomical Labeling}: Converting CC400 ROI indices to anatomical region names (e.g., "ROI 204" → specific brain structure) for clinical interpretability.
  \item \textbf{Temporal Pattern Analysis}: Examining learned temporal convolutional kernels to identify discriminative time-frequency characteristics.
  \item \textbf{Behavioral Correlation}: Correlating identified biomarkers with ASD symptom severity scores (ADOS, ADI-R) when available.
\end{itemize}

These analyses are essential for:
\begin{enumerate}
  \item Validating that the model learns biologically meaningful patterns rather than dataset artifacts.
  \item Enabling clinical trust and adoption by providing explainable predictions.
  \item Generating neuroscientific hypotheses about ASD pathophysiology.
  \item Identifying potential therapeutic targets based on altered connectivity patterns.
\end{enumerate}

\subsection{State-of-the-Art Comparison}

Table~\ref{tab:sota_comparison} compares BrainGAT Evolution with state-of-the-art models on the ABIDE dataset for ASD vs. control classification.

\begin{table}[H]
\centering
\caption{Comparison with State-of-the-Art Methods on ABIDE}
\label{tab:sota_comparison}
\begin{tabular}{llccc}
\toprule
\textbf{Method} & \textbf{Architecture Type} & \textbf{Temporal} & \textbf{Accuracy (\%)} & \textbf{AUC} \\
\midrule
BrainGNN~\cite{li2021braingnn} & ROI-aware GCN & No & 71--74 & 0.78 \\
DMSGCN~\cite{wen2022dmsgcn} & Population GCN & No & 68--70 & 0.72 \\
Original BrainGAT & Multi-head GAT & No & 52--54 & 0.55 \\
BrainNetCNN~\cite{kawahara2017brainnetcnn} & Edge-to-Node CNN & No & 65--67 & 0.68 \\
FCN-GCN~\cite{ktena2018distance} & Functional GCN & No & 63--65 & 0.66 \\
\midrule
\textbf{BrainGAT Evolution} & \textbf{Temporal-Spatial GAT} & \textbf{Yes} & \textbf{62--67} & \textbf{0.69--0.73} \\
\quad + Multi-scale & Multi-scale graphs & Yes & 63 (val) & 0.692 \\
\quad + Rich edge features & 4D edge features & Yes & 67 (test) & 0.725 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[noitemsep]
  \item BrainGAT Evolution achieves performance comparable to established methods like DMSGCN and FCN-GCN, despite being purely imaging-based without phenotype covariates.
  \item The temporal branch provides a 5--10\% improvement over the static BrainGAT baseline, demonstrating the value of modeling dynamic connectivity.
  \item While BrainGNN achieves higher accuracy (71--74\%), it uses ROI-aware embeddings and hierarchical pooling which we plan to integrate in future work.
  \item The test set performance (67\%) with AUC 0.725 indicates good generalization with proper subject-level splitting.
  \item Multi-scale spatial graphs with rich 4D edge features (correlation, partial correlation, mutual information, phase synchrony) contribute to robust performance across validation and test sets.
\end{itemize}
...
\begin{itemize}[noitemsep]
  \item \textbf{Evaluation protocol debugging:} Recognising that extremely high validation accuracy must be questioned and traced back to splitting strategy rather than celebrated.
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Biomedical interpretability analyses}: Conducting in-depth visualization and interpretation of learned attention weights to identify which brain regions and functional networks are most discriminative for ASD classification, including:
  \begin{itemize}
    \item Hemisphere-specific analysis (left vs. right brain contributions)
    \item Regional importance maps highlighting critical ROIs from the CC400 atlas
    \item Functional network analysis (default mode, salience, executive control networks)
    \item Attention weight visualization across GAT layers to understand information flow
    \item Correlation with established ASD neuroscience findings (e.g., reduced long-range connectivity, altered default mode network)
  \end{itemize}
  \item Temporal kernel interpretability to identify discriminative time-frequency patterns in the fMRI signal.
...
The work emphasised scientific rigour: refining the evaluation protocol, enforcing subject-level splits, implementing triple validation, and addressing numerical stability. The final BrainGAT Evolution model in \href{https://github.com/paymybills/IITKPG_BrainGAT/blob/master/new/BrainGAT_Evolution.ipynb}{\texttt{BrainGAT\_Evolution.ipynb}} provides a realistic and well-documented benchmark, obtaining accuracies in line with the current state of the art and establishing a solid foundation for further investigation.
...
Di Martino, A., et al. (2014). The autism brain imaging data exchange: towards a large-scale evaluation of the intrinsic brain architecture in autism. \emph{Molecular Psychiatry}, 19(6), 659-667.

\bibitem{li2021braingnn}
Li, X., et al. (2021). BrainGNN: Interpretable Brain Graph Neural Network for fMRI Analysis. \emph{Medical Image Analysis}, 74, 102233.

\bibitem{wen2022dmsgcn}
Wen, G., et al. (2022). DMSGCN: Dynamic Multi-scale Graph Convolutional Networks for Autism Spectrum Disorder Diagnosis. \emph{IEEE Transactions on Medical Imaging}.

\bibitem{velivckovic2017graph}
Veličković, P., et al. (2017). Graph Attention Networks. \emph{International Conference on Learning Representations (ICLR)}.

\bibitem{kawahara2017brainnetcnn}
Kawahara, J., et al. (2017). BrainNetCNN: Convolutional neural networks for brain networks; towards predicting neurodevelopment. \emph{NeuroImage}, 146, 1038-1049.

\bibitem{ktena2018distance}
Ktena, S. I., et al. (2018). Distance Metric Learning using Graph Convolutional Networks: Application to Functional Brain Networks. \emph{Medical Image Computing and Computer Assisted Intervention (MICCAI)}.
